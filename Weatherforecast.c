...
void makePrediction(WeatherData *data) {
    ...
    if ((data->temperature > 25.0 && data->temperature <=50.0) &&  data->humidity >0.0 && data->humidity <60.0 ) {
		...
		strcpy(prediction,"Sunny \u2600");//prediction with unicode
        	...
        	printf("Prediction: Sunny and Windy \u2600 \U0001F32A\n");
		strcpy(prediction,"Sunny and Windy \u2600 \U0001F32A");//prediction with unicode	
	}
	else{
    	printf("!!  Prediction Failed Sensor data exceeding limit!!\n");
    	strcpy(prediction,"!!  Prediction Failed Sensor data exceeding limit!!");
   	 }
    } 
    else if ((data->temperature > 15.0 && data->temperature <=25.0 )&&  data->humidity >0.0 && data->humidity <65.0  ) {
       if(data->wind >0.0 && data->wind < 15.0){
		...
		printf("Prediction: Cloudy \u2601\n");
        	...
        	printf("Prediction: Cloudy and Windy \u2601 \U0001F32A\n");
		...
		printf("Prediction: Winter \u2744\n");
        	...
        	printf("Prediction: Winter and Windy \u2744\U0001F32A\n");
